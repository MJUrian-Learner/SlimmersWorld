export interface Equipment {
  ...
  export function getEquipmentByType(type: Equipment["type"]): Equipment[] {
    return Object.values(equipmentDatabase).filter((equipment) => equipment.type === type)
  }
  
  export function updateEquipmentStatus(qrCode: string, status: Equipment["status"]): boolean {
